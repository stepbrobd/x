...
let base = "https://codeberg.org/api/v1"
...
let check code body =
  ...
  then Ok ()
    ...
    let msg =
      ...
      | _ -> body
    in
    Error (Printf.sprintf "codeberg api error (%d): %s" code msg))
;;

let exists code = code = 409 (* conflict: repo already exists *)
...
let list client ~token ~user:_ =
  let url = base ^ "/user/repos?limit=50" in
  ...
  else Error (Printf.sprintf "codeberg api error (%d): %s" code data)
;;
